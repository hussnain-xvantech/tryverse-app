...
export const Route = createFileRoute("/signup/shoppers")({
  head: () => ({
    meta: [
      { title: "Create Shopper Account — TryVerse" },
      {
        name: "description",
        content: "Sign up as a shopper to try clothes on virtually before buying.",
      },
    ],
  }),
  component: ShopperSignupPage,
});

function ShopperSignupPage() {
  ...
  return (
    <AuthShell
      ...
      subtitle="Start trying clothes on before you buy."
      footer={
        <p>
          Already have an account?{" "}
          <Link to="/login/shoppers" className="text-white font-medium hover:underline">
            Log in.
          </Link>
        </p>
      }
    >
      ...
      <form onSubmit={submit} className="space-y-4" noValidate>
        ...
        <AuthInput id="email" label="Email" type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={setEmail} error={errors.email} />
        <AuthInput
          id="password"
          label="Password"
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full justify-center !py-3 mt-2 disabled:opacity-60">
          {loading ? "Creating account…" : (<>Create Shopper Account <ArrowRight size={16} /></>)}
        </button>
        <ul className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2 text-[12px] text-white/60">
          {["Free to start", "No credit card required", "Clothing-only try-on"].map((t) => (
